 ...
 *
 * Physical bracelets are generated by `scripts/export-bracelets.mjs`
 * (run via `npm run export:bracelets`) and printed from disk. Each SVG
 * has "№ / N" and a "batch of <word>" line baked into the ink, so
 * bracelets printed under the old pool size will be inconsistent with
 * bracelets printed under the new one.
 *
 * Checklist when you bump this number:
 *   □ Update `POOL_SIZE` and `BATCH_WORDS` in
 *     `scripts/export-bracelets.mjs` so newly-generated SVGs match.
