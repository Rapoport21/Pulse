...
const Checkbox: React.FC<{ checked: boolean; color: string }> = ({ checked, color }) => (
  <div style={{ width: 20, height: 20, borderRadius: RADIUS.sm, background: checked ? `${color}18` : COLORS.surface, border: `1.5px solid ${checked ? color : COLORS.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: 1, transition: `all ${MOTION.fast}s ease` }}>
    {checked && <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} style={{ fontSize: 15, color, lineHeight: 1 }}>{'\u2713'}</motion.span>}
  </div>
);
        ...
        return (
          <React.Fragment key={s}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <div style={{ width: 22, height: 22, borderRadius: RADIUS.full, background: done ? COLORS.accent : cur ? COLORS.accentDim : COLORS.surface, border: `1.5px solid ${done || cur ? COLORS.accent : COLORS.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontFamily: FONTS.mono, fontWeight: 600, color: done ? COLORS.textPrimary : cur ? COLORS.accent : COLORS.textMuted }}>
                {done ? '\u2713' : i + 1}
              </div>
  ...
  const renderPatientList = () => (
    <div style={{ padding: SPACE.base, display: 'flex', flexDirection: 'column', gap: SPACE.base }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <motion.button onClick={toggleAll} whileTap={{ scale: 0.96 }} style={{ background: 'transparent', border: `1px solid ${COLORS.border}`, borderRadius: RADIUS.sm, padding: `4px ${SPACE.sm}px`, cursor: 'pointer', fontFamily: FONTS.mono, fontSize: 13, color: COLORS.accent, letterSpacing: 0.5, textTransform: 'uppercase' as const }}>
        ...
        return (
          <div key={p.id}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: SPACE.md, paddingBottom: SPACE.sm, borderBottom: `1px solid ${COLORS.border}` }}>
              ...
              <motion.button onClick={() => setSbarNotes((prev) => ({ ...prev, [p.id]: aiDraft(p) }))} whileTap={{ scale: 0.95 }} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: `4px ${SPACE.sm}px`, background: `${COLORS.accent}10`, border: `1px solid ${COLORS.accent}40`, borderRadius: RADIUS.sm, cursor: 'pointer', fontFamily: FONTS.mono, fontSize: 13, color: COLORS.accent, letterSpacing: 0.5, textTransform: 'uppercase' as const }}>
                <BrainCircuit size={15} /> AI Draft
              </motion.button>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: SPACE.sm }}>
              {SBAR_SECTIONS.map((sec) => (
                <div key={sec.key}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: SPACE.sm, marginBottom: 4 }}>
                    <span style={{ width: 20, height: 20, borderRadius: RADIUS.sm, background: `${sec.tone}18`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: FONTS.mono, fontSize: 13, fontWeight: 700, color: sec.tone }}>{sec.prefix}</span>
    ...
    return (
      <div style={{ padding: SPACE.base, display: 'flex', flexDirection: 'column', gap: SPACE.base }}>
            ...
            <TacticalCard key={p.id} padding="md">
              ...
              {SBAR_SECTIONS.map((sec) => (
                <div key={sec.key} style={{ marginBottom: SPACE.sm }}>
                  <span style={{ fontFamily: FONTS.mono, fontSize: 13, fontWeight: 700, color: sec.tone }}>[{sec.prefix}]</span>
        ...
        <div style={{ display: 'flex', gap: SPACE.sm }}>
          <TacticalButton variant="ghost" onClick={handleCopy} icon={<ClipboardCopy size={17} />}>Copy</TacticalButton>
          <TacticalButton variant="ghost" onClick={handlePrint} icon={<Printer size={17} />}>Print</TacticalButton>
        </div>
        <TacticalButton variant="primary" fullWidth onClick={handleSend} icon={<Send size={17} />}>Send to Incoming Team</TacticalButton>
      </div>
    );
  };
        ...
        <motion.div key="handoff-composer" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: MOTION.fast }} style={{ position: 'fixed', inset: 0, zIndex: Z.modal, background: COLORS.bg, display: 'flex', flexDirection: 'column', fontFamily: FONTS.sans, color: COLORS.textPrimary, overflow: 'hidden', paddingTop: 'env(safe-area-inset-top)' }}>
          ...
          {!sent && (
            <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, display: 'flex', alignItems: 'center', gap: SPACE.sm, padding: `${SPACE.md}px ${SPACE.base}px`, paddingBottom: `max(${SPACE.md}px, env(safe-area-inset-bottom))`, background: `linear-gradient(180deg, ${COLORS.bg}00 0%, ${COLORS.bg} 20%)`, borderTop: `1px solid ${COLORS.border}`, zIndex: Z.modal + 1 }}>
              {stepIndex > 0 && <TacticalButton variant="ghost" onClick={goBack} icon={<ArrowLeft size={17} />}>Back</TacticalButton>}
              <div style={{ flex: 1 }} />
              {step !== 'review' && <TacticalButton variant="primary" onClick={goNext} disabled={!canProceed} icon={<ArrowRight size={17} />}>{step === 'sbar' ? 'Review' : 'Next'}</TacticalButton>}
            </div>
          )}
        </motion.div>
      )}
