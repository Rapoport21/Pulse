...
const METRICS: Array<{ label: string; value: string; trend: 'up' | 'down' | 'stable'; tone: 'ok' | 'warn' | 'crit' | 'info'; icon: React.ReactNode }> = [
  { label: 'Census', value: '42/52', trend: 'up', tone: 'info', icon: <Users size={16} /> },
  { label: 'Capacity', value: '81%', trend: 'up', tone: 'warn', icon: <Gauge size={16} /> },
  { label: 'MEWS>3', value: '2 pts', trend: 'stable', tone: 'crit', icon: <Activity size={16} /> },
  { label: 'Avg Wait', value: '34min', trend: 'up', tone: 'warn', icon: <Timer size={16} /> },
];
...
const TrendIcon: React.FC<{ trend: 'up' | 'down' | 'stable'; color: string }> = ({ trend, color }) =>
  trend === 'up' ? <TrendingUp size={14} color={color} /> : trend === 'down' ? <TrendingDown size={14} color={color} /> : <Minus size={14} color={color} />;
...
const VitalChip: React.FC<{ label: string; value: string; tone: 'ok' | 'warn' | 'crit' }> = ({ label, value, tone }) => {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
      <span style={{ fontFamily: FONTS.mono, fontSize: 10, fontWeight: 500, letterSpacing: '0.12em', color: COLORS.textMuted, textTransform: 'uppercase' }}>{label}</span>
      <span style={{ fontFamily: FONTS.mono, fontSize: 13, fontWeight: 600, color: c }}>{value}</span>
    </div>
  );
};
      ...
      {open && (
        ...
        >
          ...
          <HudStrip side="top" fixed>
            <button onClick={handleClose} style={closeBtnStyle}><X size={17} /></button>
            ...
            <div style={{ padding: SPACE.base, display: 'flex', flexDirection: 'column', gap: SPACE.base }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: SPACE.sm }}>
                ...
                <TacticalButton variant="secondary" size="sm" onClick={handleRegenerate} disabled={regenerating}
                  icon={<RefreshCw size={15} style={regenerating ? { animation: 'spin 1s linear infinite' } : undefined} />}
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: SPACE.sm, marginBottom: SPACE.md }}>
                  <BrainCircuit size={17} color={COLORS.accent} />
                  ...
                  <AnimatePresence mode="wait">
                    {!ready || regenerating ? (
                      <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                        ...
                        <RefreshCw size={17} color={COLORS.info} style={{ animation: 'spin 1.4s linear infinite' }} />
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: SPACE.sm }}>
                  {PRIORITY_ACTIONS.map((action, i) => (
                    ...
                    >
                      ...
                      <TacticalButton variant="primary" size="sm" onClick={() => handleAct(action)} icon={<ChevronRight size={14} />} style={{ flexShrink: 0 }}>Act</TacticalButton>
                    </motion.div>
                  ))}
                </div>
              </div>
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: SPACE.sm, marginBottom: SPACE.md }}>
                  <Activity size={16} color={COLORS.crit} />
                      ...
                      <TacticalCard padding="md">
                        <div style={{ display: 'flex', alignItems: 'flex-start', gap: SPACE.md }}>
                          ...
                          <div style={{ width: 36, height: 36, borderRadius: RADIUS.sm, background: `${mewsCol(pt.mews)}12`, border: `1.5px solid ${mewsCol(pt.mews)}`, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                            <span style={{ fontFamily: FONTS.mono, fontSize: 17, fontWeight: 700, color: mewsCol(pt.mews), lineHeight: 1 }}>{pt.mews}</span>
                            <span style={{ fontFamily: FONTS.mono, fontSize: 8, fontWeight: 500, color: mewsCol(pt.mews), letterSpacing: '0.1em', textTransform: 'uppercase', lineHeight: 1, marginTop: 1 }}>MEWS</span>
                          </div>
                    ...
                    return (
                      <motion.button key={i} onClick={() => setExpandedHistory(expanded ? null : i)} whileTap={{ scale: 0.99 }}
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: SPACE.md, width: '100%' }}>
                          ...
                          <ChevronDown size={15} color={COLORS.textMuted} style={{ transition: `transform ${MOTION.fast}s ease`, transform: expanded ? 'rotate(180deg)' : 'rotate(0deg)', flexShrink: 0 }} />
                        </div>
                        <AnimatePresence>
                          {expanded && (
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: SPACE.sm, padding: `${SPACE.lg}px 0 ${SPACE.base}px` }}>
                <BrainCircuit size={15} color={COLORS.textDim} />
