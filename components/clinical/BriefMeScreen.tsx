 ...
 *
 * Core PULSE differentiator: the AI synthesizes all hospital data into a
 * role-aware narrative briefing — the morning huddle, powered by an AI
 * that has read every piece of data in the hospital.
 *
 * Sections:
 *   1. Briefing Header   — title, timestamp, confidence, regenerate
 *   2. Executive Summary  — narrative overview with typing effect
 *   3. Key Metrics Strip  — census, capacity, MEWS, avg wait
 *   4. Priority Actions   — AI-recommended actions by urgency
 *   5. Patient Spotlight  — patients needing attention
 *   6. Forecast           — next 90-minute predictions
 *   7. Briefing History   — previous briefings
 *
 * Props:
...
const EXEC_SUMMARY =
  'ED operating at 82% capacity with 3 critical patients. ICU has 1 bed available — expect 2 transfers from ED within the next 90 minutes. Staffing is adequate for current census but night shift will be 2 RNs short in ICU. Blood bank inventory is sufficient; OR utilization is at 67% with 2 rooms available.';
...
const severityLabel = (s: Severity): string => s.toUpperCase();

const toneFromSeverity = (s: Severity): 'crit' | 'warn' | 'info' | 'neutral' => {
  switch (s) {
...
const mewsColor = (m: number): string =>
  m >= 5 ? COLORS.crit : m >= 3 ? COLORS.warn : COLORS.ok;

const TrendIcon: React.FC<{ trend: 'up' | 'down' | 'stable'; color: string }> = ({ trend, color }) => {
  if (trend === 'up') return <TrendingUp size={11} color={color} />;
  if (trend === 'down') return <TrendingDown size={11} color={color} />;
  return <Minus size={11} color={color} />;
};

const toneColor = (t: 'ok' | 'warn' | 'crit' | 'info'): string => {
  switch (t) {
...
export const BriefMeScreen: React.FC<BriefMeScreenProps> = ({ open, onClose, showToast }) => {
  ...
  const handleAct = useCallback((action: PriorityAction) => {
    showToast(`Action initiated: ${action.text.slice(0, 50)}...`);
  }, [showToast]);
            ...
            <div style={{ padding: SPACE.base, display: 'flex', flexDirection: 'column', gap: SPACE.base }}>
              ...
              <TacticalCard
                ...
                style={{
                  position: 'relative',
                  overflow: 'hidden',
                  background: `linear-gradient(135deg, ${COLORS.surface} 0%, ${COLORS.surfaceElev} 100%)`,
                }}
              >
                  ...
                  >
                    <TacticalCard padding="md">
                      <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: SPACE.xs }}>
                        <span style={{ color: toneColor(m.tone) }}>{m.icon}</span>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                        ...
                        <TrendIcon trend={m.trend} color={toneColor(m.tone)} />
                      </div>
                    </TacticalCard>
                  </motion.div>
                ))}
              ...
              <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: SPACE.sm }}>
                  {PRIORITY_ACTIONS.map((action, i) => (
                    ...
                    >
                      <StatusPill
                        label={severityLabel(action.severity)}
                        tone={toneFromSeverity(action.severity)}
                        size="xs"
                      />
                        ...
                        <div style={{ display: 'flex', alignItems: 'flex-start', gap: SPACE.md }}>
                          ...
                          <div
                            style={{
                              ...
                              height: 36,
                              borderRadius: RADIUS.sm,
                              background: `${mewsColor(pt.mews)}12`,
                              border: `1.5px solid ${mewsColor(pt.mews)}`,
                              display: 'flex',
                              flexDirection: 'column',
                              alignItems: 'center',
                              justifyContent: 'center',
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: SPACE.sm, marginBottom: SPACE.xs }}>
                              ...
                              <TrendIcon trend={pt.trend} color={mewsColor(pt.mews)} />
                            </div>
                          ...
                          <button
                            onClick={() => showToast(`Opening chart for ${pt.name}`)}
                            style={{
                              display: 'flex',
                              alignItems: 'center',
                              justifyContent: 'center',
                      ...
                      >
                        <div
                          style={{
                            ...
                            background: item.confidence >= 85 ? COLORS.ok : item.confidence >= 70 ? COLORS.warn : COLORS.textMuted,
                            boxShadow: item.confidence >= 85 ? `0 0 4px ${COLORS.ok}` : undefined,
                            flexShrink: 0,
                            marginTop: 6,
                          }}
                        />
                    ...
                    return (
                      <motion.button
                        ...
                        style={{
                          display: 'flex',
                          flexDirection: 'column',
                          gap: isExpanded ? SPACE.sm : 0,
                          padding: `${SPACE.md}px ${SPACE.base}px`,
                          background: isExpanded ? COLORS.surfaceElev : COLORS.surface,
              ...
              <div style={{
                ...
                justifyContent: 'center',
                gap: SPACE.sm,
                padding: `${SPACE.lg}px 0 ${SPACE.base}px`,
              }}>
...
const VitalChip: React.FC<{ label: string; value: string; tone: 'ok' | 'warn' | 'crit' }> = ({
  label,
  value,
  tone,
}) => {
